...
Use Array.prototype.find() to identify the inventor with the first name 'Ada' from an array of inventor objects
- You have an array of objects, each representing an inventor with various properties including their first name.
- Utilize the Array.prototype.find() method to locate the object for the inventor with the first name 'Ada'.
...
Use the Array.prototype.map() method to reformat each name in the 'people' array. The goal is to convert names from "Last, First" format to "First Last" format.
...
// Complete the exercise in the space below:
const now = new Date();
isAdultPresent = devs.some((dev) => dev.year <= now.getFullYear() - 18);
// Check your work:
...
// Complete the exercise in the space below:
isEveryone19OrOlder = devs.every((dev) => dev.year <= now.getFullYear() - 19);
// Check your work:
...
Use Array.prototype.find() to identify and retrieve the comment object with a specific ID 823423 from an array of comment objects.
...
Determine the index of the comment that has a specific ID 123523 in an array of comment objects.
...
Calculate the combined lifespan of all the inventors using Array.prototype.reduce()
- Each object in the array includes these properties: 'first', 'last', 'year' (birth year), and 'passed' (year of death).
- Use the Array.prototype.reduce() method to calculate the sum of the total years lived by all the inventors.
...
totalYearsLived = inventors.reduce(
  (total, num) => total + num.passed - num.year,
  0
);
// Check your work:
...
Hints:
- Inside the reduce function, check if the travel method already exists as a key in your accumulator object. If it does, increment its count. If not, add it to the object and give it a value of 1.
...
// Complete the exercise in the space below:
travelMethodCounts = travelMethods.reduce((acc, cur) => {
